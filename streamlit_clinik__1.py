...
if st.session_state.get('show_analytics'):
    ...
    st.markdown("## Вывод")
    st.markdown("Из анализа данных видно, что в начале периода наблюдались **пропуски в посещении** в поликлинику в том же количестве, что и активные записи, что отрицательно сказывалось на результативности работы. Однако, во второй половине периода мы видим снижения пропусков до уровня в **среднем 20%** от общего числа записей.")
    st.markdown("Учитывая, что пропуски могут быть обусловлены факторами, не учтенными в наших данных, и с учетом точности модели в **75%**, мы можем дальше сократить пропуски до уровня около **10%** если внедрим эту модель в проиводство. Это может иметь значительное положительное влияние на работу поликлиники, позволяя составлять более точное расписание и эффективно управлять финансовыми ресурсами.")
    #3d
